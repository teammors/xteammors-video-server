...
@Component
public class Seedance2VideoGenerator {

    private static final Logger log = LoggerFactory.getLogger(Seedance2VideoGenerator.class);

    private final ThirdPartyServiceConfig serviceConfig;
    private OkHttpClient client;
    private final ObjectMapper objectMapper = new ObjectMapper();

    public Seedance2VideoGenerator(ThirdPartyServiceConfig serviceConfig) {
    ...
    public void init() {
        ...
        log.info("Seedance video generator initialized");
    }
    ...
    public void destroy() {
        if (client != null) {
            ...
            log.info("Seedance video generator shutdown");
        }
    }
    ...
    public GenerationResult generateVideo(VideoGenerationRequest request, String serviceKey) {
        String modelId = serviceConfig.getModelId(serviceKey);
        String apiKey = serviceConfig.getApiKey(serviceKey);
        String apiUrl = serviceConfig.getApiUrl(serviceKey);
        
        log.info("Starting video generation with model: {}, service: {}, resolution: {}", modelId, serviceKey, request.getResolution());
        ...
        try {
            ...
            try (Response createResponse = client.newCall(createHttpRequest).execute()) {
                ...
                log.debug("Create response JSON: {}", createResponseBody);
                ...
                return pollTaskStatus(taskId, serviceKey);
            }
    ...
    public String getModelTaskId(VideoGenerationRequest request, String serviceKey) {
        ...
        log.info("Creating video task with model: {}, service: {}, resolution: {}", modelId, serviceKey, request.getResolution());
        ...
        try {
            ...
            log.error("Create video task failed", e);
    ...
    public GenerationResult pollForExistingTask(String modelTaskId, String serviceKey) {
        return pollTaskStatus(modelTaskId, serviceKey);
    }
    ...
    private GenerationResult pollTaskStatus(String taskId, String serviceKey) {
        String apiKey = serviceConfig.getApiKey(serviceKey);
        String apiUrl = serviceConfig.getApiUrl(serviceKey);
            ...
            while (retries < maxPollRetries) {
                ...
                try (Response getResponse = client.newCall(getHttpRequest).execute()) {
                    if (!getResponse.isSuccessful()) {
                        ...
                        log.warn("Poll task failed: {} - {}, will retry...", getResponse.code(), errorBody);
                    ...
                    log.debug("Poll response JSON: {}", getResponseBody);
                    ...
                    if ("succeeded".equalsIgnoreCase(status)) {
                        ...
                        log.debug("Task {} status: {}, waiting...", taskId, status);
                    }
                }
